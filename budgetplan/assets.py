...
class AssetList(UserList):
    """
    This class can be used to create (nested) AssetList objects

    Arguments:
        name: the name of the AssetList object
        data: a list containing Asset and/or AssetList objects
    """
    ...
    def to_yaml(cls, dumper, mapping):
        """
        This method defines the yaml representation of an assetlist
        """
        return dumper.represent_mapping(
            cls.yaml_tag, {"name": mapping.name, "data": mapping.data}
        )

    @classmethod
    def from_yaml(cls, loader, node):
        """
        This method instantiates an AssetList object from a yaml file
        """
    ...
    def agg(self, f_agg, r_init=None):
        """
        This method iterates over an (nested) AssetList and return an
        aggregated result using the aggregation function `f_agg`

        Arguments:
            f_agg: aggregation function able to handle 2 arguments
              r: intermediate aggregation result,
              o: object to be added to the aggregation
            r_init: initial value to which aggregation result will be added.
                    The aggregation function `f_agg` should be able to handle
                    this type as the intial value.
        """
    ...
    def get_balance_eom(self):
        def agg_balance_eom(r, a):
            asset_balance_sheet_df = (
                a.balance_sheet_df.assign(asset_name=a.params["name"]).reset_index()
...
class Asset:
    ...
    def __init__(self, name, params_asset):
        ...
        # self.params_general = params_general = {"inflation": 0.00, "ym_end": "2040-12"}
    ...
    def to_yaml(cls, dumper, data):
        return dumper.represent_mapping(
            cls.yaml_tag, {"name": data.name, "params": data.params}
        )
...
class AccountAsset(Asset):
    ...
    def compute_balance_sheet(self):
        ...
        for i in range(0, ym_balance.shape[0]):
            if i > 0:
                ym_balance["balance_bom"].iloc[i] = ym_balance["balance_eom"].iloc[
                    i - 1
                ]
            ym_balance["interest"].iloc[i] = ym_balance["balance_bom"].iloc[i] * (
                (1 + ym_balance["interest_rate_y"].iloc[i]) ** (1 / 12) - 1
            )
            ym_balance["balance_eom"].iloc[i] = (
                ym_balance["balance_bom"].iloc[i]
                + ym_balance["interest"].iloc[i]
                + ym_balance["transaction"].iloc[i]
            )
